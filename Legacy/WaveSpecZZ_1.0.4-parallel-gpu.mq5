...
#import "mt-bridge.dll"
  ...
  int  gpu_submit_fft_real_forward(const double &in[], int len, int &job_id);
  int  gpu_try_get_result(int job_id, double &out[], int out_len, int &ready);
  int  gpu_free_job(int job_id);
  ...
  int  gpu_spectral_denoise(const double &spec_in[], int spectrum_len, int method, double threshold, double beta, int iterations, double &out[], int out_len);
  int  gpu_spectral_upscale(const double &spec_in[], int spectrum_len, double factor, int mode, int normalize, double &out[], int out_cap, int &out_len);
  int  gpu_apply_mask(const double &spec_in[], int spectrum_len, const double &mask_in[], int mask_len, int mask_is_complex, int mode, double &out[], int out_len);
  int  gpu_spectral_phase_unwrap(const double &spec_in[], int spectrum_len, int method, double &out[], int out_len);
#import
                            ...
                            const ENUM_TIMEFRAMES source_tf)
{
    ...
    const int saved_points = MathMin(point_count, MAX_ZIG_POINTS);
    g_zig_mask_count = saved_points;
    for(int idx = 0; idx < saved_points; ++idx)
    {
        g_zig_mask_indices[idx] = point_indices[idx];
        g_zig_mask_values[idx] = point_values[idx];
    }
    g_zig_mask_ready = (g_zig_mask_count >= 2);
...
input int  InpFFTWindow   = 65536;  // Janela da FFT 64-128-256-512-1024-2048-4096-8196-16384-32768-65536-131072-262144
...
input group "=== GPU PREPROCESSAMENTO (TEMPO) ==="
input bool   InpEnableZeroPad         = false;   // Inserir zeros antes/depois
input int    InpPadLeft               = 0;       // Amostras ? esquerda
input int    InpPadRight              = 0;       // Amostras ? direita
input bool   InpEnableResample        = false;   // Reamostrar s?rie
input double InpResampleFactor        = 1.0;     // Fator (>0)
input double InpResampleCutoff        = 0.45;    // Cutoff normalizado (0-0.5)
input int    InpResampleMethod        = 0;       // M?todo de resample
input bool   InpEnableDcRemoval       = false;   // Remover DC
input int    InpDcMode                = 0;       // 0=sub m?dia, 1=IIR
input double InpDcAlpha               = 0.98;    // Alpha DC (modo 1)

input group "=== GPU PREPROCESSAMENTO (ESPECTRO) ==="
input bool   InpEnableSpectralDenoise = false;   // Denoise espectral
input int    InpDenoiseMethod         = 0;       // M?todo de denoise
input double InpDenoiseThreshold      = 0.10;    // Threshold
input double InpDenoiseBeta           = 0.75;    // Beta
input int    InpDenoiseIterations     = 1;       // Itera??es
input bool   InpEnableSpectralUpscale = false;   // Upscale espectral
input double InpSpectralUpscaleFactor = 1.0;     // Fator (>1)
input int    InpSpectralUpscaleMode   = 0;       // Modo
input int    InpSpectralUpscaleNormalize = 1;    // Normalizar
input bool   InpEnableSpectralMask    = false;   // Aplicar m?scara
input int    InpMaskMode              = 0;       // 0=flat,1=band,2=high,3=low
input double InpMaskLow               = 0.15;    // Faixa baixa
input double InpMaskHigh              = 0.85;    // Faixa alta
input bool   InpMaskUseZigZag         = true;    // Blend com ZigZag
input int    InpMaskZigZagWidth       = 2;       // Largura (bins)
input double InpMaskZigZagBlend       = 0.65;    // 0-1 blend
input bool   InpEnablePhaseUnwrap     = false;   // Calcular fase desenrolada
input int    InpPhaseMethod           = 0;       // M?todo unwrap
input bool   InpEnableSpectralConvolution = false;   // Convolu??o espectral
input int    InpConvolutionPeriod     = 32;      // Ciclo alvo (barras)
input double InpConvolutionBandwidth  = 0.04;    // Largura normalizada
input double InpConvolutionGain       = 1.0;     // Ganho do kernel
input bool   InpEnableSpectralCorrelation = false; // Correla??o espectral opcional
...
double g_preproc_kernel[];
double g_zig_mask_temp[];

#define MAX_ZIG_POINTS 128
int    g_zig_mask_indices[MAX_ZIG_POINTS];
double g_zig_mask_values[MAX_ZIG_POINTS];
...
bool NeedsGpuPreprocessing()
  {
   return InpEnableZeroPad || InpEnableResample || InpEnableDcRemoval ||
          InpEnableSpectralDenoise || InpEnableSpectralUpscale ||
          InpEnableSpectralMask || InpEnablePhaseUnwrap ||
          InpEnableSpectralConvolution || InpEnableSpectralCorrelation;
  }
...
void LogPreprocessFailure(const string &op, const int status)
  {
   PrintFormat("[WaveSpecZZ] %s falhou (%d)", op, status);
  }
   ...
   if(!InpEnableResample)
     {
      if(targetLen > 0 && seriesLen != targetLen)
        {
         const double fallback_factor = (double)targetLen / (double)MathMax(1, seriesLen);
...
void BuildMaskArray(const int len)
  {
   ...
   for(int i = 0; i < len; ++i)
     {
      ...
      switch(InpMaskMode)
        {
         case 1: value = (ratio >= low && ratio <= high) ? 1.0 : 0.0; break;
         case 2: value = (ratio >= low) ? 1.0 : 0.0; break;
         case 3: value = (ratio <= high) ? 1.0 : 0.0; break;
   ...
   if(InpMaskUseZigZag && g_zig_mask_ready && blend > 0.0)
     {
      ...
      for(int idx = 0; idx < g_zig_mask_count - 1; ++idx)
        {
         ...
         if(delta <= 0)
            continue;
         const double freq = 1.0 / (double)delta;
         const int center = (int)MathRound(freq * len);
      ...
      for(int i = 0; i < len; ++i)
        {
        g_preproc_mask[i] = (1.0 - blend) * g_preproc_mask[i] +
                            blend * g_zig_mask_temp[i];
        }
     }
  }
...
bool SubmitGpuFftJob(const int len)
  {
   if(len <= 0)
      return false;
   EnsureArraySize(g_preproc_fft, len);
   g_pending_fft_len = len;
   g_pending_fft_job_id = 0;
   const int status = gpu_submit_fft_real_forward(g_preproc_pipeline, len, g_pending_fft_job_id);
   if(status != ALGLIB_STATUS_OK)
     {
...
bool CollectGpuFftJob()
  {
   if(g_pending_fft_job_id <= 0 || g_pending_fft_len <= 0)
      return false;
   int ready = 0;
   const int job_id = g_pending_fft_job_id;
   const int status = gpu_try_get_result(job_id,
                                          g_preproc_fft,
                                          g_pending_fft_len,
                                          ready);
   ...
   if(status == ALGLIB_STATUS_OK && ready != 0)
     {
      const int free_status = gpu_free_job(job_id);
...
void BuildConvolutionKernel(const int len)
  {
   if(len <= 0)
      return;
   EnsureArraySize(g_preproc_kernel, len);
   ...
   for(int i = 0; i < len; ++i)
     {
      ...
      g_preproc_kernel[i] = gain * MathExp(-delta * delta / sigma);
     }
  }
...
bool ApplySpectralStages(int &seriesLen)
  {
   if(!(InpEnableSpectralDenoise || InpEnableSpectralUpscale || InpEnableSpectralMask || InpEnablePhaseUnwrap))
      return true;

   EnsureArraySize(g_preproc_fft, seriesLen + 16);
   ArrayResize(g_preproc_fft, seriesLen);
   if(!EnsureFftResult(seriesLen))
      return false;

   int spectrumLen = seriesLen;
   int status = ALGLIB_STATUS_OK;
...
bool RunGpuPreprocessing(double &series[], const int targetLen)
  {
   if(!NeedsGpuPreprocessing())
      return true;

   CopySeriesToPipeline(series, targetLen);
   int seriesLen = targetLen;

   if(!EnsureWaveformGpuConfigured(seriesLen))
      return false;
                ...
                const int &spread[])
{
    ...
    for(int i = start; i < end_index && !IsStopped(); i++)
    {
        ...
        {
            Print("[WaveSpecZZ] Step: phase analysis completed (phase, unwrap, group delay)");
