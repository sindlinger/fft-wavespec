...
input int  InpFFTWindow   = 32768;  // Janela da FFT 64-128-256-512-1024-2048-4096-8196-16384-32768-65536-131072-262144
input int  InpMinPeriod   = 9;    // Per?odo m?nimo do ciclo a detectar
input int  InpMaxPeriod   = 52;    // Per?odo m?ximo do ciclo a detectar
input int  InpTrendPeriod = 8;  // Per?odo do filtro de tend?ncia (maior que InpMaxPeriod)
...
input ENUM_TIMEFRAMES        InpZigZagLowerTF1 = PERIOD_M1;   // Timeframe ZigZag alternativo 1
input ENUM_TIMEFRAMES        InpZigZagLowerTF2 = PERIOD_M1;   // Timeframe ZigZag alternativo 2
input FFT_ZIGZAG_SERIES_MODE InpZigZagSeriesMode = ZIGZAG_ALTERNATING; // Modo de constru??o da s?rie ZigZag
input int InpHistoryChunk   = 5000;     // Barras historicas processadas por chamada
...
input bool   InpShowWave3  = false;
input bool   InpShowWave4  = false;
input bool   InpShowWave5  = false;
input bool   InpShowWave6  = false;
input bool   InpShowWave7  = false;
input bool   InpShowWave8  = false;
input bool   InpShowWave9  = false;
input bool   InpShowWave10 = false;
input bool   InpShowWave11 = false;
input bool   InpShowWave12 = false;
     ...
     //   processed_bars++;
        ...
        if(g_gpu_waveform_session_initialized)
        {
            ...
            if(g_gpu_waveform_session_initialized)
            {
                ...
                {
                    PrintFormat("[GPU] gpu_fft_real_forward failed: %d na barra %d. Reverting to CPU FFT.", status_fft, i);
                    g_gpu_waveform_session_initialized = false;
        ...
        if(!logged_fft)
        {
            PrintFormat("[WaveSpecZZ] Step: FFT executed via %s (length=%d)", used_gpu_fft ? "GPU" : "CPU", InpFFTWindow);
            logged_fft = true;
        }
