...
bool EnsureZigZagHandleForTf(const ENUM_TIMEFRAMES tf)
{
    ...
    ResetLastError();
    int handle = iCustom(_Symbol, resolved, "WaveSpecZZ/ZigZag", InpZigZagDepth, InpZigZagDeviation, InpZigZagBackstep);
...
ENUM_TIMEFRAMES GetActiveZigZagTimeframe()
{
    ...
    {
        case ZIG_SOURCE_LOWER1: return ResolveTimeframe(InpZigZagLowerTF1);
        case ZIG_SOURCE_LOWER2: return ResolveTimeframe(InpZigZagLowerTF2);
                            ...
                            const ENUM_TIMEFRAMES source_tf)
{
    ...
    else
    {
        ...
        for(int j = 0; j < InpFFTWindow; ++j)
        {
            datetime bar_time = time[start_pos + j];
            int shift = iBarShift(_Symbol, resolved_tf, bar_time);
            if(shift == -1)
                continue;

            double value = 0.0;
            if(CopyBuffer(handle, 0, shift, 1, temp) > 0)
                value = temp[0];

            if(value == 0.0 || !MathIsValidNumber(value))
            {
                if(CopyBuffer(handle, 1, shift, 1, temp) > 0 && temp[0] != 0.0 && MathIsValidNumber(temp[0]))
                    value = temp[0];
                else if(CopyBuffer(handle, 2, shift, 1, temp) > 0 && temp[0] != 0.0 && MathIsValidNumber(temp[0]))
                    value = temp[0];
            }
...
input int  InpFFTWindow   = 4096;  // Janela da FFT (64, 128, 256)
...
input FFT_ZIGZAG_SOURCE_MODE InpZigZagSource = ZIG_SOURCE_CURRENT; // Timeframe do ZigZag utilizado
input ENUM_TIMEFRAMES        InpZigZagLowerTF1 = PERIOD_M15;       // Timeframe ZigZag alternativo 1
input ENUM_TIMEFRAMES        InpZigZagLowerTF2 = PERIOD_M5;        // Timeframe ZigZag alternativo 2
input FFT_ZIGZAG_SERIES_MODE InpZigZagSeriesMode = ZIGZAG_CONTINUOUS; // Modo de constru??o da s?rie ZigZag
...
int OnInit()
{
    ...
    if(InpAppliedPrice == FFT_PRICE_ZIGZAG)
    {
        if(!EnsureZigZagHandleForTf((ENUM_TIMEFRAMES)_Period))
            return(INIT_FAILED);

        if(InpZigZagLowerTF1 != PERIOD_CURRENT)
        {
            if(!EnsureZigZagHandleForTf(InpZigZagLowerTF1))
                return(INIT_FAILED);
        }

        if(InpZigZagLowerTF2 != PERIOD_CURRENT && InpZigZagLowerTF2 != InpZigZagLowerTF1)
        {
            if(!EnsureZigZagHandleForTf(InpZigZagLowerTF2))
                return(INIT_FAILED);
        }
    }
