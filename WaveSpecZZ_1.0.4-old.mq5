...
#property indicator_plots   74  // 24 visuais + 48 alternados + 1 confluence + Kalman

#import "mt-bridge.dll"
  int  gpu_init(int device_index, int stream_count);
  void gpu_shutdown(void);
  int  gpu_extract_cycles(const double &series[], int len,
                          int top_k,
                          double min_period,
                          double max_period,
...
input int  InpFFTWindow   = 8196;  // Janela da FFT 64-128-256-512-1024-2048-4096-8196-16384-32768-65536-131072-262144
...
input int InpHistoryChunk   = 50000;     // Barras historicas processadas por chamada
input int InpHistoryMaxBars = 100000;   // Limite de barras do histórico (0 = todo)
...
input int    InpTopCyclesToShow = 12;  // Quantos ciclos espectrais exibir (1-12)
input bool   InpLogDebug        = false; // Log detalhado do pipeline GPU
...
double WaveKalman[];  // Wave resultante do blend Kalman dos ciclos ativos
...
#define MAX_KALMAN_CYCLES 32
const int kGpuCycleAttrStride = 8;
enum GpuCycleAttrField
  {
   GPU_ATTR_AMPLITUDE = 0,
   GPU_ATTR_FREQUENCY = 1,
   GPU_ATTR_PERIOD    = 2,
   GPU_ATTR_PHASE     = 3,
   GPU_ATTR_ETA_BARS  = 4,
   GPU_ATTR_ETA_SECS  = 5,
   GPU_ATTR_ENERGY    = 6,
   GPU_ATTR_COHERENCE = 7
  };

//--- Estado do filtro de Kalman (peso/covariância por ciclo)
double g_kalman_weights[MAX_KALMAN_CYCLES];
double g_kalman_covariance[MAX_KALMAN_CYCLES];
bool   g_kalman_ready = false;

//--- Cache do extrator de ciclos GPU
double g_gpu_cycle_attrs[];
int    g_gpu_cycle_rows = 0;
int    g_gpu_cycle_capacity = 0;
...
input group "=== KALMAN WAVE BLEND ==="
input bool   InpEnableKalmanBlend      = true;   // Ativa o blend dos ciclos via Kalman
input int    InpKalmanMaxCycles        = 12;     // Quantidade de ciclos usados no estado
input double InpKalmanProcessNoise     = 0.25;   // Vari?ncia do ru?do de processo (Q)
input double InpKalmanMeasurementNoise = 4.0;    // Vari?ncia do ru?do de medi??o (R)
input double InpKalmanInitVariance     = 25.0;   // Vari?ncia inicial dos pesos
input color  InpKalmanColor            = clrWhite; // Cor da Wave Kalman
...
double CalculateScientificETASeconds(int fft_index, double phase_length_seconds, double current_phase_progress, double seconds_per_bar)
{
    ...
    const int slot = FindGpuCycleSlotByIndex(fft_index, InpFFTWindow);
    if(slot >= 0)
    {
        const int base = slot * kGpuCycleAttrStride;
        const double eta_secs = g_gpu_cycle_attrs[base + GPU_ATTR_ETA_SECS];
        if(MathIsValidNumber(eta_secs) && eta_secs > 0.0)
            return eta_secs;
        const double eta_bars = g_gpu_cycle_attrs[base + GPU_ATTR_ETA_BARS];
        if(MathIsValidNumber(eta_bars) && eta_bars > 0.0)
            return eta_bars * seconds_per_bar;
    }

    if(phase_length_seconds <= 0.0)
        return 0.0;

    double progress = current_phase_progress;
    if(progress < 0.0) progress = 0.0;
    if(progress > 1.0) progress = 1.0;
    return MathMax(0.0, (1.0 - progress) * phase_length_seconds);
}
...
double ComputeETA_RealFFT(int fft_index, double period_bars, int n, double seconds_per_bar)
{
    ...
    if(period_bars <= 0.0)
        return 0.0;
    return MathMax(0.0, period_bars * seconds_per_bar);
}
...
void ShowCycleListDebug()
{
    ...
    for(int c = 0; c < 12; c++)
    {
        ...
        int n = InpFFTWindow;
        if(n <= 0) n = 1;
        double freq_cyc_per_bar = (double)bin / (double)n;
        double num_cycles_window = (double)bin;
...
{
    return MathMax(1, MathMin(12, InpTopCyclesToShow));
}
...
int OnInit()
{
    ...
    SetIndexBuffer(24, WaveKalman,        INDICATOR_DATA);
...
// Alternating ETA/Leak/State/SIG buffers per cycle (plots 25-72)
    ...
    PlotIndexSetInteger(24, PLOT_DRAW_TYPE, InpEnableKalmanBlend ? DRAW_LINE : DRAW_NONE);
    PlotIndexSetString(24, PLOT_LABEL, "WaveKalman");
    PlotIndexSetDouble(24, PLOT_EMPTY_VALUE, EMPTY_VALUE);
    PlotIndexSetInteger(24, PLOT_LINE_COLOR, 0, InpKalmanColor);
...
bool input_visibility[12] =
    ...
    const bool show_legacy = !InpEnableKalmanBlend;
    for(int idx = 0; idx < 12; ++idx)
      {
       ...
       const bool draw_wave = show_legacy && g_wave_visible[idx];
       PlotIndexSetInteger(plot_wave, PLOT_DRAW_TYPE, draw_wave ? DRAW_LINE : DRAW_NONE);
       ...
       PlotIndexSetInteger(plot_wave, PLOT_LINE_COLOR, 0, wave_colors[idx]);
        ...
        // períodos ficam ocultos por padrão; mantemos essa configuração
...
{
    const double init_var = MathMax(1e-6, InpKalmanInitVariance);
    for(int c = 0; c < 12; ++c)
    {
        g_kalman_weights[c] = 0.0;
        g_kalman_covariance[c] = init_var;
    }
    g_kalman_ready = false;
}

void ClearLegacyWaveData(int bar_idx)
{
    if(bar_idx < 0)
        return;
    WaveBuffer1[bar_idx]  = EMPTY_VALUE; WaveBuffer2[bar_idx]  = EMPTY_VALUE;
    WaveBuffer3[bar_idx]  = EMPTY_VALUE; WaveBuffer4[bar_idx]  = EMPTY_VALUE;
    WaveBuffer5[bar_idx]  = EMPTY_VALUE; WaveBuffer6[bar_idx]  = EMPTY_VALUE;
    WaveBuffer7[bar_idx]  = EMPTY_VALUE; WaveBuffer8[bar_idx]  = EMPTY_VALUE;
    WaveBuffer9[bar_idx]  = EMPTY_VALUE; WaveBuffer10[bar_idx] = EMPTY_VALUE;
    WaveBuffer11[bar_idx] = EMPTY_VALUE; WaveBuffer12[bar_idx] = EMPTY_VALUE;

    WavePeriodBuffer1[bar_idx]  = EMPTY_VALUE; WavePeriodBuffer2[bar_idx]  = EMPTY_VALUE;
    WavePeriodBuffer3[bar_idx]  = EMPTY_VALUE; WavePeriodBuffer4[bar_idx]  = EMPTY_VALUE;
    WavePeriodBuffer5[bar_idx]  = EMPTY_VALUE; WavePeriodBuffer6[bar_idx]  = EMPTY_VALUE;
    WavePeriodBuffer7[bar_idx]  = EMPTY_VALUE; WavePeriodBuffer8[bar_idx]  = EMPTY_VALUE;
    WavePeriodBuffer9[bar_idx]  = EMPTY_VALUE; WavePeriodBuffer10[bar_idx] = EMPTY_VALUE;
    WavePeriodBuffer11[bar_idx] = EMPTY_VALUE; WavePeriodBuffer12[bar_idx] = EMPTY_VALUE;

    ColorBuffer1[bar_idx]  = 0.0; ColorBuffer2[bar_idx]  = 0.0;
    ColorBuffer3[bar_idx]  = 0.0; ColorBuffer4[bar_idx]  = 0.0;
    ColorBuffer5[bar_idx]  = 0.0; ColorBuffer6[bar_idx]  = 0.0;
    ColorBuffer7[bar_idx]  = 0.0; ColorBuffer8[bar_idx]  = 0.0;
    ColorBuffer9[bar_idx]  = 0.0; ColorBuffer10[bar_idx] = 0.0;
    ColorBuffer11[bar_idx] = 0.0; ColorBuffer12[bar_idx] = 0.0;

    EtaCycle1[bar_idx]  = 0.0; EtaCycle2[bar_idx]  = 0.0;
    EtaCycle3[bar_idx]  = 0.0; EtaCycle4[bar_idx]  = 0.0;
    EtaCycle5[bar_idx]  = 0.0; EtaCycle6[bar_idx]  = 0.0;
    EtaCycle7[bar_idx]  = 0.0; EtaCycle8[bar_idx]  = 0.0;
    EtaCycle9[bar_idx]  = 0.0; EtaCycle10[bar_idx] = 0.0;
    EtaCycle11[bar_idx] = 0.0; EtaCycle12[bar_idx] = 0.0;
}

void UpdateKalmanWave(int bar_idx, const double &cycle_vals[], int cycle_count, double measurement)
{
    if(bar_idx < 0 || bar_idx >= ArraySize(WaveKalman))
        return;
    if(cycle_count <= 0)
    {
        WaveKalman[bar_idx] = 0.0;
        return;
    }
    const double process_var = MathMax(1e-9, InpKalmanProcessNoise);
    const double measurement_var = MathMax(1e-9, InpKalmanMeasurementNoise);
    if(!g_kalman_ready)
        g_kalman_ready = true;

    double residual = measurement;
    double innovation_var = measurement_var;
    double cov_tmp[MAX_KALMAN_CYCLES];
    double weight_tmp[MAX_KALMAN_CYCLES];

    for(int idx = 0; idx < cycle_count; ++idx)
    {
        g_kalman_covariance[idx] += process_var;
        cov_tmp[idx] = g_kalman_covariance[idx];
        weight_tmp[idx] = g_kalman_weights[idx];
        residual -= cycle_vals[idx] * weight_tmp[idx];
        innovation_var += cycle_vals[idx] * cycle_vals[idx] * cov_tmp[idx];
    }
    ...
    for(int idx = 0; idx < cycle_count; ++idx)
    {
        const double H = cycle_vals[idx];
        const double cov = cov_tmp[idx];
        const double K = (cov * H) / innovation_var;
        const double updated_weight = weight_tmp[idx] + K * residual;
        const double updated_cov = (1.0 - K * H) * cov;
        g_kalman_weights[idx] = updated_weight;
        g_kalman_covariance[idx] = MathMax(updated_cov, 1e-9);
        blended += updated_weight * H;
    }
    WaveKalman[bar_idx] = blended;
}

void SortCycleArrayDescending(CycleInfo &cycles[], int count)
{
...
int GetRequestedGpuCycles()
{
    int requested = GetTopCyclesToShow();
    requested = MathMax(1, requested);
    requested = MathMin(requested, InpKalmanMaxCycles);
    requested = MathMin(requested, MAX_KALMAN_CYCLES);
    return requested;
}
...
void EnsureGpuCycleCapacity(int rows)
{
    if(rows <= 0)
    {
        ArrayResize(g_gpu_cycle_attrs, 0);
        g_gpu_cycle_capacity = 0;
        return;
    }
    if(g_gpu_cycle_capacity >= rows)
        return;
    ArrayResize(g_gpu_cycle_attrs, rows * kGpuCycleAttrStride);
    g_gpu_cycle_capacity = rows;
}

bool FetchGpuCycleAttributes(const double &series[], int len, double seconds_per_bar)
{
    if(len <= 0)
    {
        g_gpu_cycle_rows = 0;
    ...
    const int request = GetRequestedGpuCycles();
    EnsureGpuCycleCapacity(request);
    int out_rows = 0;
    const int status = gpu_extract_cycles(series,
                                          len,
                                          request,
                                          (double)InpMinPeriod,
                                          (double)InpMaxPeriod,
                                          ...
                                          request,
                                          out_rows);
    if(status != ALGLIB_STATUS_OK || out_rows <= 0)
    {
        g_gpu_cycle_rows = 0;
        if(InpLogDebug)
            PrintFormat("[WaveSpecZZ] gpu_extract_cycles falhou (status=%d len=%d)", status, len);
        return false;
    }
    g_gpu_cycle_rows = MathMin(out_rows, request);
    return true;
}

void PopulateGpuSpectralBuffers(int bar_idx, int rows)
{
    if(bar_idx < 0)
        return;
    const int top_cycles = GetTopCyclesToShow();
    for(int slot = 0; slot < 12; ++slot)
    {
        if(slot < rows && slot < top_cycles)
        {
            const int base = slot * kGpuCycleAttrStride;
            const double amplitude = g_gpu_cycle_attrs[base + GPU_ATTR_AMPLITUDE];
            const double phase     = g_gpu_cycle_attrs[base + GPU_ATTR_PHASE];
            const double period    = g_gpu_cycle_attrs[base + GPU_ATTR_PERIOD];
            const double value     = amplitude * MathSin(phase);
            WriteWaveSlot(slot, bar_idx, value, period, (value >= 0.0 ? 1.0 : 0.0));
        }
        else
        {
            WriteWaveSlot(slot, bar_idx, EMPTY_VALUE, 0.0, 0.0);
        }
    }
}

int CollectGpuKalmanValues(int rows, double &out_vals[])
{
    const int limit = MathMin(MathMin(rows, GetRequestedGpuCycles()), MathMin(InpKalmanMaxCycles, MAX_KALMAN_CYCLES));
    int used = 0;
    for(int slot = 0; slot < limit; ++slot)
    {
        ...
        const double amplitude = g_gpu_cycle_attrs[base + GPU_ATTR_AMPLITUDE];
        const double phase     = g_gpu_cycle_attrs[base + GPU_ATTR_PHASE];
        const double contrib   = amplitude * MathSin(phase);
        if(!MathIsValidNumber(contrib))
            continue;
        out_vals[used++] = contrib;
    }
    return used;
}

int BuildGpuCycleInfo(int fft_window_len, CycleInfo &out_cycles[])
{
    ArrayResize(out_cycles, 0);
    if(fft_window_len <= 0 || g_gpu_cycle_rows <= 0)
        return 0;
    int cycle_count = 0;
    for(int slot = 0; slot < g_gpu_cycle_rows; ++slot)
    {
        const int base = slot * kGpuCycleAttrStride;
        const double period = g_gpu_cycle_attrs[base + GPU_ATTR_PERIOD];
        if(!MathIsValidNumber(period) || period <= 0.0)
            continue;
        const double idx_raw = (double)fft_window_len / period;
        if(!MathIsValidNumber(idx_raw) || idx_raw <= 0.0)
            continue;
        int idx = (int)MathRound(idx_raw);
        if(idx <= 0)
            continue;
        idx = MathMax(1, MathMin(idx, fft_window_len - 1));
        double energy = g_gpu_cycle_attrs[base + GPU_ATTR_ENERGY];
        if(!MathIsValidNumber(energy) || energy <= 0.0)
        {
            const double amplitude = g_gpu_cycle_attrs[base + GPU_ATTR_AMPLITUDE];
            energy = amplitude * amplitude;
        }
        ArrayResize(out_cycles, cycle_count + 1);
        out_cycles[cycle_count].index = idx;
        out_cycles[cycle_count].power = energy;
        ++cycle_count;
    }
    return cycle_count;
}

int FindGpuCycleSlotByIndex(int fft_index, int fft_window_len)
{
    if(fft_index <= 0 || fft_window_len <= 0 || g_gpu_cycle_rows <= 0)
        return -1;
    int best_slot = -1;
    int best_diff = fft_window_len;
    for(int slot = 0; slot < g_gpu_cycle_rows; ++slot)
    {
        const int base = slot * kGpuCycleAttrStride;
        const double period = g_gpu_cycle_attrs[base + GPU_ATTR_PERIOD];
        if(!MathIsValidNumber(period) || period <= 0.0)
            continue;
        int slot_index = (int)MathRound((double)fft_window_len / period);
        if(slot_index <= 0)
            continue;
        const int diff = MathAbs(slot_index - fft_index);
        if(diff < best_diff)
        {
            best_diff = diff;
            best_slot = slot;
...
void WriteWaveSlot(int slot, int bar_idx, double wave_value, double period_value, double color_flag)
{
    switch(slot)
    {
        case 0:  WaveBuffer1[bar_idx]  = wave_value; WavePeriodBuffer1[bar_idx]  = period_value; ColorBuffer1[bar_idx]  = color_flag; break;
        case 1:  WaveBuffer2[bar_idx]  = wave_value; WavePeriodBuffer2[bar_idx]  = period_value; ColorBuffer2[bar_idx]  = color_flag; break;
        case 2:  WaveBuffer3[bar_idx]  = wave_value; WavePeriodBuffer3[bar_idx]  = period_value; ColorBuffer3[bar_idx]  = color_flag; break;
        case 3:  WaveBuffer4[bar_idx]  = wave_value; WavePeriodBuffer4[bar_idx]  = period_value; ColorBuffer4[bar_idx]  = color_flag; break;
        case 4:  WaveBuffer5[bar_idx]  = wave_value; WavePeriodBuffer5[bar_idx]  = period_value; ColorBuffer5[bar_idx]  = color_flag; break;
        case 5:  WaveBuffer6[bar_idx]  = wave_value; WavePeriodBuffer6[bar_idx]  = period_value; ColorBuffer6[bar_idx]  = color_flag; break;
        case 6:  WaveBuffer7[bar_idx]  = wave_value; WavePeriodBuffer7[bar_idx]  = period_value; ColorBuffer7[bar_idx]  = color_flag; break;
        case 7:  WaveBuffer8[bar_idx]  = wave_value; WavePeriodBuffer8[bar_idx]  = period_value; ColorBuffer8[bar_idx]  = color_flag; break;
        case 8:  WaveBuffer9[bar_idx]  = wave_value; WavePeriodBuffer9[bar_idx]  = period_value; ColorBuffer9[bar_idx]  = color_flag; break;
        case 9:  WaveBuffer10[bar_idx] = wave_value; WavePeriodBuffer10[bar_idx] = period_value; ColorBuffer10[bar_idx] = color_flag; break;
        case 10: WaveBuffer11[bar_idx] = wave_value; WavePeriodBuffer11[bar_idx] = period_value; ColorBuffer11[bar_idx] = color_flag; break;
        case 11: WaveBuffer12[bar_idx] = wave_value; WavePeriodBuffer12[bar_idx] = period_value; ColorBuffer12[bar_idx] = color_flag; break;
    }
}
                ...
                const int &spread[])
{
    ...
    PrintFormat("[WaveSpecZZ] Step: history window prepared (chunk=%d history_start=%d history_limit=%d)", chunk, history_start, history_limit);
    ...
    for(int i = start; i < end_index && !IsStopped(); i++)
    {
        ...
        if(!EnsureWaveformGpuConfigured(InpFFTWindow))
        {
            ...
            {
                Print("[GPU] Configuration failed. cycle extractor skipped.");
                gpu_warning_logged = true;
            }
            if(InpEnableKalmanBlend)
                PopulateGpuSpectralBuffers(i, 0);
            continue;
        }

        const bool gpu_success = FetchGpuCycleAttributes(detrended_data, InpFFTWindow, seconds_per_bar);
        if(InpEnableKalmanBlend)
            PopulateGpuSpectralBuffers(i, g_gpu_cycle_rows);
        if(!gpu_success || g_gpu_cycle_rows <= 0)
        {
            if(!gpu_warning_logged && InpLogDebug)
                PrintFormat("[WaveSpecZZ] gpu_extract_cycles retornou 0 linhas na barra %d (len=%d)", i, InpFFTWindow);
            gpu_warning_logged = true;
            if(InpEnableKalmanBlend)
                WaveKalman[i] = 0.0;
            continue;
        ...
        int cycle_count = BuildGpuCycleInfo(InpFFTWindow, all_cycles);
        if(cycle_count <= 0)
        {
            if(InpEnableKalmanBlend)
                WaveKalman[i] = 0.0;
        ...
        SortCycleArrayDescending(all_cycles, cycle_count);

        if(!logged_cycle_scan)
        {
            PrintFormat("[WaveSpecZZ] Step: cycle scan completed via GPU (candidates=%d)", cycle_count);
            logged_cycle_scan = true;
        }

        if(InpEnableKalmanBlend)
        {
            double kalman_vals[MAX_KALMAN_CYCLES];
            const int used_cycles = CollectGpuKalmanValues(g_gpu_cycle_rows, kalman_vals);
            if(used_cycles > 0)
                UpdateKalmanWave(i, kalman_vals, used_cycles, close[i]);
            else
                WaveKalman[i] = 0.0;
            if(!logged_kalman)
            {
                PrintFormat("[WaveSpecZZ] Kalman blend active (GPU extractor, cycles=%d, Q=%.3f, R=%.3f)",
                             used_cycles, InpKalmanProcessNoise, InpKalmanMeasurementNoise);
                logged_kalman = true;
            }
            continue;
        }
...
//--- 9. FOLLOW THE FIRST (v7.55) ---
        ...
        {
            Print("[WaveSpecZZ] Step: FollowTheFirst flow updated (exit/entry)");
        ...
        if(InpExportToCSV && i == 0)
        {
            ...
            if(!logged_export)
            {
                Print("[WaveSpecZZ] Step: CSV export executed for latest history chunk");
                logged_export = true;
            }
        }
