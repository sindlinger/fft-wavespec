...
input bool               InpForceBarSeries     = true;     // ignora modo tick e usa barras
...
string StatusToString(const int status);
...
string StatusToString(const int status)
{
    switch(status)
      {
       case ALGLIB_STATUS_OK:               return "OK";
       case ALGLIB_STATUS_BAD_ARGS:         return "BAD_ARGS";
       case ALGLIB_STATUS_BACKEND_UNAVAILABLE: return "BACKEND_UNAVAILABLE";
       case ALGLIB_STATUS_TIMEOUT:          return "TIMEOUT";
       case ALGLIB_STATUS_INTERNAL_ERROR:   return "INTERNAL";
       case ALGLIB_STATUS_NOT_READY:        return "NOT_READY";
       case ALGLIB_STATUS_NO_MEM:           return "NO_MEM";
...
bool IsCycleBetter(int lhs_index, int rhs_index)
{
                         ...
                         double &seconds_per_bar)
{
    ...
    bool ready = false;
    const bool want_tick_series = (InpUseTickSeries && !InpForceBarSeries);
    if(want_tick_series)
      {
      ...
      {
       if(InpUseTickSeries && InpForceBarSeries && !g_tick_warning_logged)
         {
          WavePrint("[Wave4EA] Tick resampling desativado por configuracao; usando serie de barras");
          g_tick_warning_logged = true;
         }
       if(g_tick_mode_active)
          PublishStatusHud();
                           ...
                           const string &preset_text)
{
    ...
    const bool tick_mode_active = (InpUseTickSeries && !InpForceBarSeries && g_tick_mode_active);
    const string mode_info = tick_mode_active ? "tick" : (InpUseTickSeries && !tick_mode_active ? "tick(wait)" : "bars");
                ...
                const int &spread[])
{
    ...
    const int warmup_start = MathMax(fft_window_len - 1, 0);
    if(rates_total <= warmup_start)
    ...
    int start = warmup_start;
    if(prev_calculated > warmup_start)
        start = MathMax(prev_calculated - 1, warmup_start);

    bool backlog_blocked = false;
    ...
    if(backlog_blocked && InpLogDebug)
        WavePrint("[Wave4EA] Jobs pendentes na GPU; aguardando resultados antes de enviar novas janelas");
